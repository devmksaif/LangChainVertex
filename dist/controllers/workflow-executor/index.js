...
async function resolveCredential(userId, service, name) {
...
async function executeWorkflow(req, res) {
    ...
    try {
        ...
        while (queue.length > 0) {
            ...
            const tool = tools_1.toolsAI.find(t => t.name === node.type);
            if (tool) {
                if (is_dry_run) {
                ...
                if (node.type === 'if_else') {
                    ...
                    if (tryEdge) {
                        ...
                        if (count === 0)
                            queue.push(tryEdge.target);
                    }
                }
                else {
                    // For other logic nodes, propagate data if they have data outputs
                    const hasDataOutput = node.data.outputs.some((o) => o.type === 'data');
                    if (hasDataOutput) {
                        const dataEdge = edges.find(e => e.target === nodeId && e.targetHandle === 'data_in');
                        if (dataEdge) {
                            context[nodeId] = context[dataEdge.source];
